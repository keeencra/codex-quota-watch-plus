...
采用项目自定义约定 `v主版本.0.修订号`，以下规则于 2026-09-13 修订，按实际产品变化判断：

- **大更新**：新增产品功能，或主动改变 App／Mac 服务的界面、交互和用户可见行为；主版本加 1，修订号归零，例如 `v2.0.1 → v3.0.0`。
- **小更新**：只修复已有产品 Bug，恢复既定行为，没有新增功能；修订号加 1，例如 `v2.0.0 → v2.0.1`。修复界面遮挡、错误导航等也属 Bug 修复，不能仅因改了界面就升主版本。
- **不升版本**：仅修改更新日志、README、配图、介绍、发布说明、维护规范或文字勘误；不改变产品行为的重构、测试和构建检查也不单独触发版本发布。这类变更记入所属版本的「文档与维护补充」，不新增迭代、标签或 Release。
- 同时新增产品功能和修 Bug 按大更新；随产品改动附带的文档和测试不另计版本。发布前先核对实际产品差异，再决定分类，不根据提交数、改动文件数或篇幅判断。
- **迭代次数独立累计**：只有有效产品版本发布才加 1。历史 v0.1.0～v2.0.0 的编号与累计 10 次记录保留，不追溯重排；旧文档误发不算有效产品版本；本次实际功能更新 v3.0.0 计为第 11 次迭代。
- 保留原发布标签和源码包，不因补充文档移动标签；当前文档和原始源码快照分别链接。中间位固定为 0，“大更新”不等于不兼容变更。
