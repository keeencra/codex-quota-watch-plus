...
本页按实际操作介绍 码伴 · CodeCompanion。所有新增截图来自当前 SwiftUI 源码的模拟器展示构建，使用独立的虚构任务与额度数据。展示构建替换数据读取并禁止提交审批，不连接个人 Mac 服务；这些替换不进入正式 App 源码。任务列表截图已更新为项目分组与侧栏名称同步版；其余截图沿用之前版本中布局未变的页面。
...
开启 [项目与任务标识](enhancements.md#按项目与任务区分结束提醒) 后，通知显示「产品开发 · 修复小组件 · 本轮已结束」。名称与侧栏同步，各会话独立匹配，Bark 保持静音；标题读取失败时显示带短标识的未命名任务。通知表示本轮处理结束，不代表整个项目完成。
