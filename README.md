...
**项目版本 v3.3.2 · Bug 修复 · 第 18 次迭代** · [查看各版本变化](CHANGELOG.md#版本索引)
...
| 手机看进展 | 按 Mac 侧栏分区查看项目与独立任务，保留分区顺序、任务标题、运行阶段和待审批请求；通过已配置的 HTTPS 连接访问 Mac |
...
**最近更新：** v3.3.2 修复分区识别与混合任务排序，兼容 iOS 26／27；v3.3.1 修复旧任务停滞误报；v3.3.0 新增每日 token 趋势与统一滚动面板；v3.2.1 修复公历日期显示；v3.2.0 改进大号组件排版、倒计时与菜单栏双币种显示。[完整变化与验证说明](CHANGELOG.md) · [当前版本源码](https://github.com/keeencra/codex-quota-watch-plus/releases/tag/v3.3.2)

## 手机与手表：分区、项目和任务保持一致

「全部任务」按 Mac 侧栏分区顺序显示，每个分区保留项目及直接放入的独立任务；空项目仍可见。重命名或移动分区后，前台刷新即可同步。「进行中与待处理」沿用同一层级筛选。

<img src="docs/assets/v3.3.2-sidebar-sections.png" width="330" alt="v3.3.2 手机分区、项目与任务层级，iOS 模拟器虚构数据">

本项目 SwiftUI 原生界面的 iOS 模拟器截图，全部数据虚构。手机支持 iOS 26 和 iOS 27，手表保留对应 watchOS 26／27 兼容；安装到 27 系统设备时使用 Xcode 27。最低部署版本未提高，具体测试范围见[系统兼容说明](docs/compatibility.md)。
...
以上是本项目 AppKit 原生视图的离屏渲染，展示同一面板的三个滚动位置；不是桌面实机截图。v3.3.2 沿用这些 Mac 面板界面。
...
以上为本项目 SwiftUI 视图离屏渲染及虚构数据；v3.3.2 沿用这套组件布局，实际系统着色效果可能不同。Mac 组件显示近期任务用量，手机组件显示今日用量，采样口径不同；菜单栏每日统计另行展示。[添加方法、刷新与其他场景](docs/widgets.md)

## 界面展示

**v3.0.1 修复演示：**「全部任务」现在保留无本地 Codex 任务的项目，空分组显示提示；Mac 更改顺序后，两端下一次前台刷新同步。下图为虚构数据行为示意，非实机截图。

![v3.0.1 空项目显示与排序同步示意图](docs/assets/v3.0.1-project-order.svg)
...
以下任务列表图为 v3.0.0 历史布局；当前分区界面见上方 v3.3.2 截图。
...
以下手表任务列表为 v3.0.0 历史布局，当前版本已按侧栏分区展示。
...
| 任务进展 | 侧栏分区、项目与独立任务、筛选、阶段与事件 | 同步分区层级及任务详情 | Hooks + 增量日志观察，处理旧状态 |
